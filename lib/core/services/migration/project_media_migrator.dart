...
/// Migrates ProjectMedia from Hive to SQLite
/// NOTE: Hive has been removed - this is now SQLite-only
class ProjectMediaMigrator {

  final ProjectMediaRepository _repository = ProjectMediaRepository();

  /// Migrate all project media from Hive to SQLite
  Future<Map<String, dynamic>> migrate() async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('❌ ProjectMedia migration failed: $e');
  ...
  Future<Map<String, dynamic>> getStatus() async {
    try {
      final sqliteCount = (await _repository.getAllProjectMedia()).length;
  ...
  Future<Map<String, dynamic>> testMigration() async {
    try {
      if (kDebugMode) {
        debugPrint('🧪 Testing ProjectMedia database access...');
      }
