...
/// Controller for managing email template editor state and operations
/// Extracted from EmailTemplateEditorScreen to separate business logic from UI
class EmailTemplateEditorController extends ChangeNotifier {
  ...
  final TextEditingController emailContentController = TextEditingController();
  ...
  void _setupListeners() {
    ...
    emailContentController.addListener(_onContentChanged);
  ...
  void _loadExistingTemplate() {
    ...
    emailContentController.text = template.emailContent;
  ...
  Future<void> _updateExistingTemplate(AppStateProvider appState) async {
    ...
    final updatedTemplate = EmailTemplate(
      ...
      emailContent: emailContentController.text.trim(),
  ...
  Future<void> _createNewTemplate(AppStateProvider appState) async {
    ...
    await appState.addEmailTemplate(template);
  }
  ...
  String getFieldDisplayName(String fieldType) {
    final appState = context.read<AppStateProvider>();
    final customFields = appState.customAppDataFields;
    return EmailTemplate.getFieldDisplayName(fieldType, customFields);
  }
